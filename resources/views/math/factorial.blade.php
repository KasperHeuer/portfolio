<x-math-tool-layout>
    ...
    @if (isset($result))
    <div class="mt-8 max-w-4xl w-full mx-auto bg-gradient-to-br from-white to-blue-50 rounded-2xl shadow-xl p-8 border border-blue-100">
        <!-- Factorial Result -->
        <div class="text-center mb-8">
            <div class="inline-block mb-3">
                <span class="text-sm font-semibold text-blue-600 uppercase tracking-wide bg-blue-100 px-3 py-1 rounded-full">
                    Factorial Result
                </span>
            </div>
        ...
        <div class="flex flex-wrap justify-center items-center gap-2 mt-4">
            @foreach ($result['sequence'] as $index => $multiplier)
                <div class="bg-gradient-to-br from-blue-500 to-indigo-600 text-white font-bold px-5 py-3 rounded-lg shadow-md hover:shadow-xl hover:scale-105 transition-all duration-200 text-lg">
                    {{ $multiplier }}
                </div>
        ...
        <div class="mt-6 pt-6 border-t border-blue-200">
            <p class="text-center text-sm text-gray-500">
                <span class="font-semibold">{{ count($result['sequence']) }}</span> numbers multiplied together
            </p>
        </div>
    </div>
    @endif
